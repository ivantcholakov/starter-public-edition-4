    ...
    <div class="card sameheight-item items" data-exclude="xs,sm,lg">
      <div class="card-header bordered">
        <div class="header-block">
          <h3 class="title"><?php echo 'All Users'; //lang('admin button users_all'); ?></h3>
        </div>
        <div class="header-block pull-right">
          <?php if ($total) : ?>
                    <a href="<?php echo $this_url; ?>/export?sort=<?php echo $sort; ?>&dir=<?php echo $dir; ?><?php echo $filter; ?>" class="btn btn-success btn-sm"><i class="icon-cloud-download icons"></i> <?php echo 'CSV Export'; //lang('admin button csv_export'); ?></a>
                <?php endif; ?> <button type="button" data-toggle="modal" data-target="#search" class="btn btn-warning btn-sm"><i class="icon-magnifier icons"></i> <?php echo 'Search';//lang('admin log search'); ?></button>
        </div>
        </div>
        ...
        <table class="table table-striped table-bordered table-hover">
          <thead>
            ...
            <tr>
                ...
                <th>
                    <a href="<?php echo current_url(); ?>?sort=status&dir=<?php echo (($dir == 'asc' ) ? 'desc' : 'asc'); ?>&limit=<?php echo $limit; ?>&offset=<?php echo $offset; ?><?php echo $filter; ?>"><?php echo 'Active';//lang('admin col status'); ?></a>
                ...
                <th><?php echo 'Actions';//lang('admin col actions'); ?></th>
            </tr>
          </thead>
          <tbody>
            <?php // data rows ?>
            <?php if ($total) : ?>
                <?php foreach ($users as $user) : ?>
                    <tr>
                        ...
                        <!-- lang('admin input active') lang('admin input inactive') -->
                            <?php echo ($user['status']) ? '<span class="label label-success">' . 'Active' . '</span>' : '<span class="label label-danger">' . 'Inactive'  . '</span>'; ?>
                        </td>
                        <td<?php echo (($sort == 'is_admin') ? ' class="sorted"' : ''); ?>>
                        <!-- lang('core text yes') : lang('core text no') -->
            ...
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                ...
                <div class="modal-header-right">
                  ...
                  <h5 class="modal-title" id="myModalLabel2"><i class="icon-magnifier icons"></i> <?php echo 'Search';//lang('admin log search'); ?></h5>
                </div>
                ...
                <div class="modal-footer"> 
                    <a href="<?php echo $this_url; ?>" class="btn btn-warning btn-sm"><?php echo 'Reset';//lang('core button reset'); ?></a> 
                    <button type="submit" name="submit" value="<?php echo lang('core button filter'); ?>" class="btn btn-primary btn-sm"><?php echo 'Filter'; //lang('core button filter'); ?></button> 
                </div>
                <?php echo form_close(); ?>
        ...
        <div class="modal fade" id="modal-<?php echo $user['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="modal-label-<?php echo $user['id']; ?>" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal"><?php echo 'Cancel'; //lang('core button cancel'); ?></button>
                        <button type="submit" class="btn btn-primary btn-delete-user btn-sm" data-id="<?php echo $user['id']; ?>"><?php echo 'Delete';//lang('admin button delete'); ?></button>
                    </div>
                </div>
            </div>
        </div>
