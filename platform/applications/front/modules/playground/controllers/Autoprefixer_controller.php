...
class Autoprefixer_controller extends Playground_Base_Controller {

    public function __construct() {

        parent::__construct();

        $title = 'CSS Autoprefixer Test';

        $this->template
            ->append_title($title)
            ->set_breadcrumb($title, site_url('playground/autoprefixer'));
        ;
    ...
    public function index() {
        ...
        if ($clear_form) {
            ...
            try {
                $output = $this->parser->parse_string($input, null, true, array('autoprefixer' => $options));
            } catch(Exception $e) {
                $output = $e->getMessage();
            }
