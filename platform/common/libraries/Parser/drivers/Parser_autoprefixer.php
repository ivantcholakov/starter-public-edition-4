...
class CI_Parser_autoprefixer extends CI_Parser_driver {
    ...
    public function initialize()
    {
        ...
        $this->config = array(
            'tmp_dir' => TMP_PATH,
            'browsers' => array('> 1%', 'last 2 versions', 'Firefox ESR'),
            'full_path' => FALSE,
        );

        if ($this->ci->config->load('parser_autoprefixer', TRUE, TRUE))
        {
            $this->config = array_merge($this->config, $this->ci->config->item('parser_autoprefixer'));
        }
        ...
        log_message('info', 'CI_Parser_autoprefixer Class Initialized');
    }
    ...
    public function parse($template, $data = array(), $return = FALSE, $options = array())
    {
        ...
        $parser = new Autoprefixer_Parser($options);
