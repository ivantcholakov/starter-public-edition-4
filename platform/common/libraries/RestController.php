...
class RestController extends \CI_Controller
{
    ...
    {
        if (
            file_exists(COMMONPATH.'config/'.$config_file.'.php')
            ||
            file_exists(APPPATH.'config/'.$config_file.'.php')
        ) {
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        if ($this->config->item('force_https') && $this->request->ssl === false) {
            $this->response([
                (string) $this->config->item('rest_status_field_name')  => false,
                (string) $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_unsupported'),
            ], self::HTTP_FORBIDDEN);
        }
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && $this->_allow === false) {
            ...
            $this->response([
                (string) $this->config->item('rest_status_field_name')  => false,
                (string) $this->config->item('rest_message_field_name') => sprintf($this->lang->line('text_rest_invalid_api_key'), $this->rest->key),
            ], self::HTTP_FORBIDDEN);
        }
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && empty($this->rest->key) === false && $this->_check_access() === false) {
            ...
            $this->response([
                (string) $this->config->item('rest_status_field_name')  => false,
                (string) $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_api_key_unauthorized'),
        ...
        if (!method_exists($this, $controller_method)) {
            $this->response([
                (string) $this->config->item('rest_status_field_name')  => false,
                (string) $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_unknown_method'),
        ...
        if ($this->config->item('rest_enable_keys') && empty($this->rest->key) === false) {
            ...
            if ($this->config->item('rest_enable_limits') && $this->_check_limit($controller_method) === false) {
                (string) $response = [$this->config->item('rest_status_field_name') => false, (string) $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_api_key_time_limit')];
            ...
            if ($authorized === false) {
                ...
                $response = [(string) $this->config->item('rest_status_field_name') => false, (string) $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_api_key_permissions')];
                $this->response($response, self::HTTP_UNAUTHORIZED);
            }
        }
        ...
        elseif ($this->config->item('rest_limits_method') == 'IP_ADDRESS' && $this->config->item('rest_enable_limits') && $this->_check_limit($controller_method) === false) {
            $response = [(string) $this->config->item('rest_status_field_name') => false, (string) $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_ip_address_time_limit')];
            $this->response($response, self::HTTP_UNAUTHORIZED);
        }
